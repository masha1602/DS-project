...
df$officialGuide<- as.numeric(df$officialGuide)
df$duration<- as.numeric(df$duration)

places = df %>% select (category, duration, officialGuide, tripAdvisor_rating, must_visit, title)
...
5.1. Another clustering (with free and duration)
...
library(readr)
library(readxl)
places = df %>% select (free, duration, category, title)
places$id= rownames(places)
places$temp_val= 1
places= places %>% spread(key='category', value= 'temp_val', fill=0)
```
...
library(ggplot2)
library(GGally)
km.out=kmeans(places, 5,nstart=20)
km.out$cluster
km.out$size
table(km.out$cluster)
sum(km.out$cluster==1)
...
class(title$clusters)
cluster1 = title %>% filter(clusters==1)
cluster2 = title %>% filter(clusters==2)
cluster3 = title %>% filter(clusters==3)
cluster4 = title %>% filter(clusters==4)
cluster5 = title %>% filter(clusters==5)
```
